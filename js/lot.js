/* =========================================================
   Sezonul în curs + Lotul echipei — fotbal / handbal
   Construiește din data/echipe.json (generat de
   scripts/actualizeaza-echipe.py):
   - rezultate recente + program + forma echipei;
   - lotul ca un deck de carduri verticale expandabile (formula
     acordeonului de la Plaja Olt): fâșia activă se lățește și
     dezvăluie fișa jucătorului; sub deck, terenul cu toate
     posturile — cel al jucătorului activ e aprins, restul gri.
   ========================================================= */
(function () {
  "use strict";
  ...
  // conținutul static al fișei fiecărui jucător (se dezvăluie la expandare)
  function cardHtml(juc) {
    var np = parteNume(juc);
    // pe fâșia pliată: numărul mare, orizontal (se citește întreg) + numele vertical
    var h = '<span class="j-strip">' +
      '<b class="j-nr">' + (juc.numar != null ? juc.numar : "&nbsp;") + "</b>" +
      '<span class="j-vert">' + esc(juc.nume) + "</span></span>";
    ...
    h += '<div class="j-copy">';
    h += '<span class="j-post">' + esc(juc.post) + "</span>";
    if (np.prenume) h += '<p class="j-prenume">' + esc(np.prenume) + "</p>";
    h += '<h3 class="j-nume">' + esc(np.familie) + "</h3>";

    var masuri = [];
    if (juc.inaltime) {
      masuri.push('<span class="masura"><i>Înălțime</i><b>' +
        (juc.inaltime / 100).toFixed(2).replace(".", ",") + " <small>m</small></b></span>");
    }
    if (juc.greutate) masuri.push('<span class="masura"><i>Greutate</i><b>' + juc.greutate + " <small>kg</small></b></span>");
    if (juc.varsta) masuri.push('<span class="masura"><i>Vârstă</i><b>' + juc.varsta + " <small>ani</small></b></span>");
    if (masuri.length) h += '<div class="j-masuri">' + masuri.join("") + "</div>";
    ...
    h += '<div class="j-fise">' + fise.map(function (f) {
    ...
    if (statCols.length) {
      h += '<div class="j-stats"><span class="jsezon">Sezon ' +
        esc((juc.sezonStats || "").replace("/20", "/")) + "</span>" + statCols.join("") + "</div>";
    }
    if (juc.pozaCredit) h += '<p class="j-credit">' + esc(juc.pozaCredit) + "</p>";
    h += "</div>";
    return h;
  }
  ...
  function deseneazaLot(e) {
    ...
    var carduri = [];
    // ordinea rămâne pe posturi (portari, apoi restul), dar fără separatoare vizibile
    ...
    function alege(juc, cardEl, anima) {
      carduri.forEach(function (c) { c.el.classList.toggle("deschis", c.el === cardEl); });
      ...
      if (activ) {
        var t = activ.getAttribute("transform"); // translate(x,y)
        if (areGsap && anima) {
