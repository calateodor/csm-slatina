...
(function () {
  ...
  function salveazaJson(fisier, modifica, mesaj) {
    ...
    return apiCiteste("v3/data/" + fisier).then(function (f) {
      var j = f.text ? JSON.parse(f.text) : {};
      j = modifica(j) || j;
      return apiScrie("v3/data/" + fisier, b64text(JSON.stringify(j, null, 2)), mesaj, f.sha)
    ...
    fisier.addEventListener("change", function () {
      ...
      optimizeaza(f).then(function (blob) {
        ...
        cititor.onload = function () {
          ...
          apiScrie("v3/" + nume, b64, "poză încărcată (" + nume + ")", null)
  ...
  function modulLot() {
    ...
    function puneCreionPeCard() {
      ...
      creion.addEventListener("click", function (ev) {
        ...
        apiCiteste("v3/data/echipe.json").then(function (f) {
